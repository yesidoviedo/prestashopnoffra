...
{if !$content_only}
	...
	<p class="alert alert-warning"><small>{l s='El despacho está sujeto a la disponibilidad del inventario. Precios sujetos a cambios sin previo aviso.'}</small></p>
{/if}
...
{else}
			...
			<tbody>
                ...
                {foreach $products as $product}
                    ...
                    {if $product.on_sale == false}
                        {include file="$tpl_dir./shopping-cart-product-line-no.tpl" productLast=$product@last productFirst=$product@first}
                    {/if}
                    {* Then the customized datas ones*}
                    {if isset($customizedDatas.$productId.$productAttributeId[$product.id_address_delivery])}
                        {foreach $customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] as $id_customization=>$customization}
				...
				<tbody>
					...
					{/if}
						<tr class="cart_discount {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
							...
							<td class="cart_discount_name" colspan="6">{$discount.name}</td>
							<td class="cart_discount_price">
	...
	<p class="cart_navigation clearfix">
		...
		<a href="http://empresasnoffra.com/20-catalogo" class="button-exclusive btn btn-default" title="{l s='Continue shopping'}">
			<i class="icon-chevron-left"></i>{l s='Continue shopping'}
		</a>
	</p>
	<div class="clear"></div>
