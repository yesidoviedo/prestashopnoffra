...
<div itemscope itemtype="https://schema.org/Product">
	...
	<div class="primary_block row">
		{if !$content_only}
			<div class="container">
				...
				<small>{l s='Para los productos en oferta sólo aplica el descuento adicional de 5%. El despacho está sujeto a la disponibilidad del inventario. Precios sujetos a cambios sin previo aviso.'}</small></p>
			</div>
		{/if}
		...
		{if $product->online_only}
			...
			<div class="block-reference">
				...
				<!-- OCULTAR CONDICION DEL PRODUCTO
			...
			<div class="box-info-product">
				<div class="content_prices clearfix text-center">
					<p>{l s="Por favor, inicie sesión para relizar su pedido."}</p><br/>
					<a href="index.php?controller=my-account" class="btn btn-primary btn-lg" target="_top">Iniciar Sesión</a>
					<hr/>
					<p>{l s="Si usted desea ser nuestro cliente, por favor contáctenos para obtener sus credenciales de acceso:"}</P><br/>
					<a href="http://empresasnoffra.com/content/7-hagase-distribuidor" class="btn btn-primary" target="_top">Solicitar acceso</a>
				</div>
			</div>
			{/if} 
			<!-- add to cart form-->
							...
							<div>
								...
								<p class="our_price_display" itemprop="offers" itemscope itemtype="https://schema.org/Offer">{strip}
									...
									{if $priceDisplay >= 0 && $priceDisplay <= 2}
										...
										{/if}
										<br/>
										<!-- PRECIO BOLIVAR SOBERANO -->
										<div id="precio_soberano">VES {($productPrice/100000)|number_format:0:",":"."},00 
										{if $tax_enabled  && ((isset($display_tax_label) && $display_tax_label == 1) || !isset($display_tax_label))}
											{if $priceDisplay == 1} <small>{l s='tax excl.'}{else} {l s='tax incl.'}</small>{/if}
										{/if}</div>
						...
						<table class="table-product-discounts">
							...
							{foreach from=$quantity_discounts item='quantity_discount' name='quantity_discounts'}
								...
								<tr id="quantityDiscount_{$quantity_discount.id_product_attribute}" class="quantityDiscount_{$quantity_discount.id_product_attribute}" data-real-discount-value="{convertPrice price = $realDiscountPrice}" data-discount-type="{$quantity_discount.reduction_type}" data-discount="{$quantity_discount.real_value|floatval}" data-discount-quantity="{$quantity_discount.quantity|intval}">
									...
									<td>
										{l s='Compre'} {$quantity_discount.quantity|intval}
										{l s='por'} 
										{if $quantity_discount.price >= 0 || $quantity_discount.reduction_type == 'amount'}
										...
										{$discountPrice=$discountPrice}
											{$qtyProductPrice=$productPriceWithoutReduction|floatval}
		...
		<div class="row">
			<div class="col-xs-12">
				{if $fichastecnicas != null && $is_starter_or_alternator == true}
					<h4>{l s="Ficha tecnica de"}: {if !isset($groups)}{$product->reference|escape:'html':'UTF-8'}{/if}</h4>
	...
	</section><!--#partfinder-spec-results-->
	{if !$content_only}
	{if (isset($quantity_discounts) && count($quantity_discounts) > 0)}
			<!-- quantity discount -->
			<!--<section class="page-product-box">
				<h3 class="page-product-heading">{l s='Volume discounts'}</h3>
				<div id="quantityDiscount2">
