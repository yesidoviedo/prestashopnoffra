...
class CartCore extends ObjectModel
{
     ...
     */
    public function getProducts($refresh = false, $id_product = false, $id_country = null, $proveedor = null )
    {
       // var_dump($proveedor);
       // die();
        ...
        //
        if (isset($proveedor)) {
            $sql->where('p.`id_supplier`= '.(int)$proveedor); 
          //  var_dump($proveedor);
          //  die();   
        }
        //
       ...
       // die();
        // Build ORDER BY
        /*$sql->orderBy('p.`id_supplier`, cp.`date_add`, cp.`id_product`, cp.`id_product_attribute` DESC');*/
        $sql->orderBy('p.`id_supplier`, p.`reference` ASC');
    ...
    */
    public function getOrderTotal($with_taxes = true, $type = Cart::BOTH, $products = null, $id_carrier = null, $use_cache = true, $supplier = null, $totalizado = null)
    {
        // Dependencies
        ...
        if (is_null($products)) {
            $param_product = false;
            $products = $this->getProducts(false,false,null,$supplier);
       //     var_dump($supplier);
        //    die();
        }
        ...
        foreach ($products as $product) {
            ...
            $null = null;
            if (is_null($totalizado) && is_null($supplier))
                { 
            ...
            elseif (($product['id_supplier'] == $supplier) && ($supplier==1)) 
                {
                $price = $price_calculator->getProductPrice(
                (int)$product['id_product'],
                $with_taxes,
            ...
            elseif ($product['id_supplier'] == $supplier ) 
            	{
                $price = $price_calculator->getProductPrice(
                (int)$product['id_product'],
                $with_taxes,
    ...
    */
     ...
     */
    public function getPackageList($flush = false, $id_supplier = NULL)
    {
        static $cache = array();
        ...
        $product_list = $this->getProducts($flush, false, null,  $id_supplier);
    ...
    */
    public function getSummaryDetails($id_lang = null, $refresh = false, $supplier = null)
    {
        $context = Context::getContext();
        if (!$id_lang) {
        ...
        $summary = array(
            ...
            'total_proveedor' => $this->getOrderTotalTotal(true, Cart::BOTH, null, null, true, 1),
            'total_tax' => $total_tax,
    ...
    */
    ...
    public function getSupplier(){
        $sql = 'SELECT distinct p.`id_supplier` as `id_supplier`, su.`name`, su.`mensaje1`, su.`mensaje2`, su.`mensaje3`
                    FROM `'._DB_PREFIX_.'cart_product` AS cp
                    LEFT JOIN `'._DB_PREFIX_.'product` AS p
                    on p.`id_product` = cp.`id_product`
                    LEFT JOIN `'._DB_PREFIX_.'supplier` AS su
                    on p.`id_supplier` = su.`id_supplier`
                    WHERE cp.`id_cart` = '.(int)$this->id.
                    ' order by p.`id_supplier`' ;
        $supplier_ids = array();
        foreach (Db::getInstance()->executeS($sql) as $row) {
            //$supplier_ids[] = array($row['id_supplier'].','.$row['name'].','.$row['mensaje1'].','.$row['mensaje2'].','.$row['mensaje3']);
            $supplier_ids[] = array('id_supplier' =>$row['id_supplier'],'name' =>$row['name'], 'mensaje1' =>$row['mensaje1'],'mensaje2' =>$row['mensaje2'],'mensaje3' =>$row['mensaje3'] );
   //         $supplier_ids[] = $row['name'];
               // ,'.$row['name'].','.$row['mensaje1'].','.$row['mensaje2'].','.$row['mensaje3']);
        }
    ...
    public function getEmailSupplier($proveedor){
        $sql = 'SELECT `email`
                    ...
                    WHERE `id_supplier` = '.$proveedor;

        $supplier_ids = Db::getInstance()->getRow($sql);
        return $supplier_ids['email'];
    }


    public function getProductos($refresh = false, $id_product = false, $id_country = null, $proveedor)
    {
...
/*        if (!$this->id) {
 ...
 //        var_dump($proveedor);
   //       die();
    ...
    public function getOrderTotalTotal($with_taxes = true, $type = Cart::BOTH, $products = null, $id_carrier = null, $use_cache = true, $supplier = null, $totalizado = null)
    {
        // Dependencies
        $address_factory    = Adapter_ServiceLocator::get('Adapter_AddressFactory');
        $price_calculator    = Adapter_ServiceLocator::get('Adapter_ProductPriceCalculator');
        ...
        $sql->from('supplier');
        $sql->where('proceso_cobro=1');
        $result = Db::getInstance()->executeS($sql);
        foreach ($result as $key => $row) {
                $supplier_ids[] = array('id_supplier' => $row['id_supplier']);
        }
      ...
      //  die();
        ...
        foreach ($supplier_ids as $supplier_id) {
            foreach ($products as $product) {
                if ($product['id_supplier'] == $supplier_id['id_supplier'] ) { 
                   // var_dump($supplier_id);
                        //die();
                    // products refer to the cart details
    ...
    public function getOrderTotalProveedor($with_taxes = true, $type = Cart::BOTH, $products = null, $id_carrier = null, $use_cache = true, $supplier = null, $totalizado = null)
    {
        // Dependencies
        $address_factory    = Adapter_ServiceLocator::get('Adapter_AddressFactory');
        $price_calculator    = Adapter_ServiceLocator::get('Adapter_ProductPriceCalculator');
...
*/
 ...
 //       foreach ($supplier_ids as $supplier_id) {
            foreach ($products as $product) {
                if ($product['id_supplier'] == $supplier ) { 
                   // var_dump($supplier_id);
                        //die();
                    // products refer to the cart details
