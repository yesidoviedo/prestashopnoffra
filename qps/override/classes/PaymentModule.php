        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            foreach ($package_list as $id_address => $packageByAddress) {
                foreach ($packageByAddress as $id_package => $package) {
                    ...
                    //cramirez : guarda la ip 
                    $order->address_ip = $order->getUserIpAddr();

                    //cramirez : guardo el agente de usuario
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (!$order_creation_failed && isset($order->id)) {
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
                        ...
                        //obtener informacion adicional del vendedor.
                        if ($row = $this->context->customer->getInformationSaler()){

                        	 if (Validate::isEmail($row['email'])) {

                        	 	$data_saler =array(
                        	 					'{firstname_saler}' => $row['firstname'],
                        						'{lastname_saler}' => $row['lastname'],
                        		);

                        	 	if (is_array($data_saler)) {
                           		 $data = array_merge($data, $data_saler);
                        		}


								//envia un correo adicional al vendedor.
								 Mail::Send(
	                                (int)$order->id_lang,
	                                'order_conf_saler',	                                
                                    sprintf(Mail::l('Order confirmation', (int)$order->id_lang), $order->id, $order->reference),
	                                $data,
	                                $row['email'],
