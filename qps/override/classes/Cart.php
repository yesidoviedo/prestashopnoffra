...
class Cart extends CartCore
{
    ...
    public function add($autodate = true, $null_values = false)
    {
        ...
        $return = ObjectModel::add($autodate, $null_values);
    ...
    public function getProducts($refresh = false, $id_product = false, $id_country = null)
    {
        ...
        $sql->select('cp.`id_product_attribute`, cp.`id_product`, cp.`quantity` AS cart_quantity, cp.id_shop, pl.`name`, p.`is_virtual`,
                        ...
                        sup.name nombre_proveedor,p.price precio_full');
        ...
        $sql->leftJoin('supplier', 'sup', 'sup.id_supplier = p.id_supplier');
        ...
        foreach ($result as &$row) {
            ...
            //mostrar los 4 columnas de los descuentos por volumen para el producto. 
           
            $res = SpecificPrice::getVolumeDiscount($row['id_product']);
            $volume_discount="";
            if ($res) {
            //aqui simplemente resaltar en amarillo el porcentaje de desacuento de acuerdo a las unidades.
                 $contador=0;
                 $row_find=0;
                 
                foreach ($res as $reg) {
                 $contador++;   
                 if((int)$row['cart_quantity'] >= (int)$reg['from_quantity']){
                    $row_find=$contador;
                 }
                }
                ...
                foreach ($res as $reg) {
                $contador++;  
                    if($contador==$row_find) {
                     $volume_discount .= "<span style='background-color:#ffff00;color:#383838'>+".(int)$reg['from_quantity']."(".((float)$reg['reduction']*100)."%)</span>&nbsp;&nbsp;&nbsp;";
                    }else{
                    $volume_discount .= "+".(int)$reg['from_quantity']."(".((float)$reg['reduction']*100)."%)&nbsp;&nbsp;&nbsp;";   
                    }
                }
            }

            $row['volume_discount']  = $volume_discount;
    ...
    */
    public function getSummaryDetails($id_lang = null, $refresh = false)
    {
        ...
        /*cramirez
        calculo y obtengo el porcentaje de impuesto que se aplico a el carrito.
        Este es un valor global debido a que las reglas de impuesto se aplican por producto.
        */
        $porcentaje_impuesto=0;
        if($base_total_tax_exc>0){
            $porcentaje_impuesto = round((($base_total_tax_inc/$base_total_tax_exc)-1)*100,2);
        }
    ...
    */
    ...
    public function getProductsSoldOutStock(){
        $list_products=array();
         foreach ($this->getProducts(true) as $product) {
            //$product['quantity']  cantidad solicitada.
            //$product['stock_quantity'] cantidad en stock.
            if($product['quantity'] > $product['stock_quantity']){
                $list_products[]= $product['name'];
            }
         }
