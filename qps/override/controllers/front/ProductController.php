...
class ProductController extends ProductControllerCore
{
    ...
    public function canonicalRedirection($canonical_url = '')
    {
        ...
        if (Validate::isLoadedObject($this->product)) {
            FrontController::canonicalRedirection($this->context->link->getProductLink($this->product));
        }
    }
    /**
    ...
    {
        //echo $this->product->reference;exit;
        ...
        if (!$this->errors) {
            ...
            //CODIGO PARA TRAER LA FICHA TECNICA AL DETALLE DEL PRODUCTO
            require_once("apps/partfinder/db/db_datamaster.php");
            require_once("apps/partfinder/models/especificaciones_model.php");

            $theirPart = $this->product->reference;
            $header = new especificaciones_model();
            $listHeader = $header->get_header_specifications();
            $dataSheet = new especificaciones_model();
            $listDataSheet = $dataSheet->get_data_sheet($theirPart);
            //Variable usada en caso de que el componente no comparta las mismas características propuestas en la consulta
            $isStarterOrAlternator = false;

            //Agrupar la cantidad de variaciones de la ficha técnica
            if ($listDataSheet != null) {
                $i = 0;
                $cpartDataSheet[$i]["cpart"] = $listDataSheet[0]["cpart"];
                foreach ($listDataSheet as $fichatecnica) {
                    if ($cpartDataSheet[$i]["cpart"] != $fichatecnica["cpart"]) {
                        $i++;
                        $cpartDataSheet[$i]["cpart"] = $fichatecnica["cpart"];
                    }
                }

                //Establecer la matriz e inicializar sus campos
                $arregloDataSheet = [];
                for ($i = 0; $i <= count($listHeader); $i++) {
                    for ($j = 0; $j <= count($cpartDataSheet); $j++) {
                        $arregloDataSheet[$i][$j] = null;
                    }
                }
                ...
                for ($i = 0; $i <= count($listHeader); $i++) {
                    for ($j = 0; $j <= count($cpartDataSheet); $j++) {
                        if ($i == 0 && $j == 0) {
                            $arregloDataSheet[$i][$j] = "Especificaciones";
                        } elseif ($i == 0) {
                            $arregloDataSheet[$i][$j] = $cpartDataSheet[$j - 1]["cpart"];
                        } elseif ($j == 0) {
                            $arregloDataSheet[$i][$j] = $listHeader[$i - 1]["Header"];
                        } else {
                            foreach ($listDataSheet as $fichatecnica) {
                                if ($fichatecnica["Header"] == $arregloDataSheet[$i][0] && $fichatecnica["cpart"] == $arregloDataSheet[0][$j]) {
                                    $isStarterOrAlternator = true;
                                    $arregloDataSheet[$i][$j] = $fichatecnica["description"];
                                }
                            }
                        }
                    }
            ...
            $this->context->smarty->assign(array(
                ...
                // ASSIGN PARA FICHA TECNICA
                'fichastecnicas' => $arregloDataSheet,
                'cf_fichatecnica' => count($listHeader), //contador de filas en ficha tecnica
                'cc_fichatecnica' => count($cpartDataSheet), //contador de columnas en ficha técnica
                'is_starter_or_alternator' => $isStarterOrAlternator,
                // Advanced part finder
