...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('.cart_quantity').live('change', function(e) {
			...
			//cramirez: Evitar que los controles para aumentar y disminuir la cantidad de items lleguen a CERO o escribir CERO
			var variacion = $(this).val() - cart_quantity[$(this).attr('rel')];
			if($.trim($(this).val())=='' || parseInt($(this).val())==0){
				variacion='indefinido';
			}

			if ($(this).val() != cart_quantity[$(this).attr('rel')])
			{
				var product = $(this).attr('rel').split('_');
				updateQty(product[0], product[1], product[2], variacion);
			}
		});
		$('.increaseqty_product, .decreaseqty_product').live('click', function(e) {
			e.preventDefault();
			var cant = $(this).parent('div').next('input').val();
			var product = $(this).attr('rel').split('_');
			var sign = '';
	...
	function updateCartProducts(products, gifts, id_address_delivery)
	{
		...
		$.each(products, function() {
			...
			cart_content += '<tr><td><img src="'+this.image_link+'" title="'+this.name+'" /></td><td>'+this.name+'<br />'+this.attributes_small+'</td><td>'+this.nombre_proveedor+'</td><td>'+this.reference+'</td><td>'+Math.round(this.precio_full*100)/100+'</td><td><input type="text" rel="'+this.id_product+'_'+this.id_product_attribute+'" class="product_unit_price" value="' + this.numeric_price + '" /></td><td>';
			...
			cart_content += '</td><td>'+this.volume_discount+'</td><td>' + formatCurrency(this.numeric_total, currency_format, currency_sign, currency_blank) + '</td></tr>';
	...
	function displaySummary(jsonSummary)
	{
		...
		$('#total_prod').html('Total: '+formatCurrency(parseFloat(jsonSummary.summary.total_products), currency_format, currency_sign, currency_blank));
...
<div class="leadin">{block name="leadin"}{/block}</div>
	<div class="panel form-horizontal" id="customer_part">
		...
		<div class="col-lg-12">
		<p class="lead"><h4>
			{l s='Look for an existing customer by entering the first four (4) letters of your name or your last name or your email or your id code. Only active clients will be displayed. To display multiple clients, enter your personal data separated by a space. Example: pedro carlos luis. Example: pedro@prestashop.com juan@prestashop.com'}
		</h4>			
		</p>
		...
		<div id="search-customer-form-group" class="form-group">
			...
			<div class="col-lg-9">
				<div class="row">
					...
					<div class="col-lg-6">
						{if $context_employee->id_profile neq 4}
...
<form class="form-horizontal" action="{$link->getAdminLink('AdminOrders')|escape:'html':'UTF-8'}&amp;submitAdd{$table|escape:'html':'UTF-8'}=1" method="post" autocomplete="off">
				...
				<table class="table" id="customer_cart">
					<thead>
						<tr>
							...
							<th><span class="title_box">{l s='Supllier'}</span></th>
							<th><span class="title_box">{l s='Reference'}</span></th>	
							<th><span class="title_box">{l s='Base price'}</span></th>						
							<th><span class="title_box">{l s='Unit price(With discount)'}</span></th>							
							<th><span class="title_box">{l s='Quantity'}</span></th>
							<th><span class="title_box">{l s='Volume discount'}</span></th>
		...
		<div class="form-group">
			<div class="col-lg-3 col-lg-offset-9">
				<div class="alert alert-info text-right">
					<span id="total_prod" class="size_l text-success"></span>
				</div>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-lg-9">
				...
				{if $context_employee->id_profile eq 4}
					disabled
				{/if}
				>
					{foreach from=$currencies item='currency'}
	...
	<div class="panel" id="address_part" style="display:none;">
		...
		{if $context_employee->id_profile neq 4}
			{assign var="disabled_link" value=""}
		{else}
			{assign var="disabled_link" value="disabled"}
		{/if}
		<div class="row">
			<div class="col-lg-12">
				<a class="fancybox btn btn-default {$disabled_link}" id="new_address" href="{$link->getAdminLink('AdminAddresses')|escape:'html':'UTF-8'}&amp;addaddress&amp;id_customer=42&amp;liteDisplaying=1&amp;submitFormAjax=1#">
					<i class="icon-plus-sign-alt"></i>
